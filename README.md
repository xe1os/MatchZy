...
- Added `.botspawn <name>`, `.delbotspawn <name>`, `.listbotspawn`, `.placebot <number> <name>`, and `.placenewbot <number> <name>` for building map-specific named spawn pools and placing utility-free bots at randomly selected, distinct saved XY positions and aim directions. `.placebot` replaces the current setup, while `.placenewbot` keeps existing bots and adds to it. `.showbotspawn` toggles green ground-aligned squares at every saved bot-spawn point and defaults to off; stand inside a visible green square and press `E` to delete only that point.
